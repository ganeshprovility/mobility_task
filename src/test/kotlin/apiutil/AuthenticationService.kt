...
class AuthenticationService(state: ScenarioData,propertiesService:PropertiesService) : APIBase(state,propertiesService) {
    val SsgUrl: String = propertiesService.getProperties().getProperty("SsgUrl")
    private val ssgSigner = SSGSigner(tenantId = TenantId, devKey = DeveloperKey, devSecret = DeveloperSecret, ssgCredentials = null)
 ...
 /*   fun postApi(state: ScenarioData): HttpResponse<String> {
        println(state);
        val headers = ssgSigner.sign(
            path = "/api-gateway/v6/api/ttools/stops",
            body = null,
